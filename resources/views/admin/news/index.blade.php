                    ...
                    @foreach($news as $new)
                      <tr>
                        ...
                        <td>{{ $new->description_uz }}</td>
                          <td>
                              <img src="/images/{{ $new->img }}" alt="" width="80px">
                          </td>
                      <td><b>{{ $new->created_at }}</b></td>
                        <td>
                            <form action="{{ route('admin.news.destroy', $new->id) }}" method="POST">

                          <a href="{{ route('admin.news.show', $new->id) }}" class="btn btn-info"><ion-icon class="fas fa-info-circle"></ion-icon></a>
                          <a href="{{ route('admin.news.edit', $new->id) }}" class="btn btn-primary"><ion-icon class="far fa-edit"></ion-icon></a>
                            @csrf
                            @method('DELETE')
